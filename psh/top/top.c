...
{
	int c, err = 0, cmd = 0, itermode = 1, run = 1, ret = EOK, tid = -1;
	...
	resthrds = threadsinfo(&tid, PH_THREADINFO_OPT_THREADCOUNT, 0, NULL) * 2;
	if (resthrds * 2 > n) {
		n = resthrds * 2;
	}
	...
	while (!psh_common.sigint && !psh_common.sigquit && !psh_common.sigstop && run) {
		...
		while ((totcnt = threadsinfo(&tid, PH_THREADINFO_ALL, n, info)) >= n) {
			n = totcnt * 2;
