...
static int psh_run(int exitable, const char *console)
{
	...
	/*
	 * POSIX: a shell that is not in the foreground stops itself with SIGTTIN
	 * until the terminal is handed over.
	 * TODO: SIGTTIN is not yet unsupported, so waiting would block forewer. Once
	 * it gets implemented, loop until we are in the fg.
	 */
	pgrp = tcgetpgrp(STDIN_FILENO);
	if ((pgrp >= 0) && (pgrp != getpgrp())) {
		(void)kill(-getpgrp(), SIGTTIN);
	}
	...
	if (psh_common.tcpid == -1) {
		if (setsid() == (pid_t)-1 && (errno != EPERM || getsid(0) != getpid())) {
	...
	if (psh_common.tcpid != -1) {
		/*
		 * Hand the terminal back to the process group that had it when we started,
		 * otherwise its foreground group would keep pointing to our dying process group.
		 */
		(void)tcsetpgrp(STDIN_FILENO, psh_common.tcpid);
	}
