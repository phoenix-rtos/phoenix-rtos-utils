...
static int psh_ls_statentry(fileinfo_t *file, const char *path, bool followSymbolic)
{
	int ret;
	struct stat st;

	ret = followSymbolic ? lstat(path, &st) : stat(path, &st);
	if (ret < 0) {
		return ret;
	}
...
static int psh_ls_readentry(fileinfo_t *file, struct dirent *dir, const char *path)
{
	...
	ret = psh_ls_statentry(file, fullname, true);
	if (ret < 0) {
...
int psh_ls(int argc, char **argv)
{
	...
	for (i = 0; i < npaths; i++) {
		ret = psh_ls_statentry(&psh_ls_common.files[nfiles], paths[i], false);
		if (ret < 0) {
			fprintf(stderr, "ls: can't access %s: no such file or directory\n", paths[i]);
			continue;
		}

		unsigned long pathlen = strlen(paths[i]);
