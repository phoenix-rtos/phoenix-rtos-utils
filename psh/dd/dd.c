...
enum { param_none = 0, param_if, param_of, param_bs,
	param_count, param_seek, param_skip };
...
static ssize_t getnumber(char *numstr)
{
	ssize_t value;
...
static int psh_dd(int argc, char **argv)
{
	...
	while (--argc > 0) {
		...
		for (par = params; par->name; par++) {
			if (strncmp(str, par->name, cp - str) == 0) {
				break;
			}
		}
	...
	outfd = ((outfile == NULL) ? fileno(stdout) : creat(outfile, 0666));
	if (outfd < 0) {
