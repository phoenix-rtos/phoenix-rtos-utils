...
/* clang-format off */
/* 512 consecutive '}' should be escaped by phfs */
static uint8_t test_table[512] = {
...
int main(int argc, char **argv)
{
	...
	/* test */
	size_t i = 0, cnt = 0;
	while (i < sizeof(test_table) / sizeof(test_table[0])) {
		cnt += test_table[i++];
	}

	if (cnt == (i * 0x7d)) {
		if ((argc > 1) && (strcmp(argv[1], "test_test") == 0)) {
			printf("success\n");
		}
	}
	/* test */
