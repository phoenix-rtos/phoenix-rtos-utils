...
class AppCollector(Collector):
    def __init__(self, gcc_tools, src_base, extra_calls, ignored_functions, *, strict=True):
    ...
    # add / remove calls based on user provided config
    def enhance_call_tree(self):
        ...
        for name in self._ignored_functions:
            try:
                ...
                logging.warning(f"failed to resolve ignored function {name}")
                errors += 1

        if errors:
            msg = f"encountered {errors} errors during extra_calls/ignored_functions resolution"
            logging.warning(msg)
            if self._strict:
                raise Exception(msg)
...
def analyze_orphans(c: AppCollector, config):
    ...
    reachable = set()
    children = collections.deque(c.symbols_by_name[name] for name in entry_points)
    ...
    for func in c.all_functions():
        ...
        size = func["size"]
        if size > 300:
            # potential entry point (300 size threshold is arbitrary)
            logging.warning(f"UNREACHABLE: {name} {size}")
    ...
    for func in by_size:
        ...
        if usage <= 20:
            # we can ~safely ignore potential function pointers with minimal stack-usage
            continue
...
def main():
    ...
    parser.add_argument("--src-base", help="Base source file used for extra-calls paths", default=None, type=Path)
    parser.add_argument("--config", type=Path)
    parser.add_argument("--keep-going", action="store_true", help="ignore (some) non-critical errors")
    ...
    c = AppCollector(
        GCCTools(gcc_base), str(src_base.absolute()), extra_calls, ignored_functions, strict=(not args.keep_going)
    )
    ...
    for thread in config["threads"]:
        ...
        if limit_type == "value":
            ...
            if not isinstance(limit, int):
                raise ValueError(f"invalid type for limit value expected: int got: {type(limit)=}")
            unit = thread["limit"].get("unit", "byte")
            if unit == "page":
    ...
    for info in results:
        ...
        if args.no_color:
            ...
            print(f"{style}{info.usage:6d}{FG_DEFAULT} | {info.limit:6d} | {info.entry}")
