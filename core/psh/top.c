...
static int top_cmdwait(double secs)
{
	struct timeval left;
	fd_set fds;
	double integral;
	double fractional = modf(secs, &integral);

	left.tv_sec = (time_t) integral;
	left.tv_usec = (time_t) fractional * 1000000;
	fflush(stdout);
	FD_ZERO(&fds);
...
int psh_top(char *arg)
{
	...
	while ((arg = psh_nextString(arg, &len)) && len) {
		if (!strcmp(arg, "-H")) {
			...
			if ((arg = psh_nextString(arg, &len)) && len) {
				...
				if (delay <= 0.0) {
					printf("top: '%s' requires double greater than 0\n", arg);
					return EOK;
				}
			} else {
	...
	prev_info = calloc(n, sizeof(threadinfo_t));
	if ((info == NULL) || (prev_info == NULL)) {
